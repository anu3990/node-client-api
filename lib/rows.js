...
Rows.prototype.query = function queryRows() {
  ...
  return queryRowsImpl(this, builtPlan, null, options, args[1]);
};
function queryRowsImpl(self, builtPlan, streamType, options, arg_1) {
  console.log('line 106')
  console.log(arg_1)
  const operation = queryRowsOperationImpl(self, builtPlan, streamType, options, arg_1);
...
function queryRowsOperationImpl(self, builtPlan, streamType, options, arg_1) {
  let sep = '?',
  ...
  switch (format) {
    ...
    default:
      ...
      // Chunked stream, promise
        ...
        acceptHeader = 'application/json'; // check with Ling
        responseType = 'single';
      }
      endpoint += sep + 'output=' + structure + '&column-types=' + columnTypes;
      if (sep === '?') { sep = '&'; }
 ...
 */
// TODO : rename query to result and queryAsStream to resultASStream and deprecate the older versions for 2.10.0
// TODO : add a new execute function similar to query (except no output for execute)
