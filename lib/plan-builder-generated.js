...
add(...args) {
    ...
    const paramdefs = [['vector1', [types.VecVector, PlanColumn, PlanParam], true, false], ['vector2', [types.VecVector, PlanColumn, PlanParam], true, false]];
    const checkedArgs = (namer !== null) ?
...
/**
    ...
    */
base64Decode(...args) {
    const paramdef = ['base64-vector', [types.XsString, PlanColumn, PlanParam], true, false];
...
/**
    ...
    */
base64Encode(...args) {
    const paramdef = ['vector1', [types.VecVector, PlanColumn, PlanParam], true, false];
...
/**
    * Returns the cosine similarity between two vectors. The vectors must be of the same dimension. Provides a client interface to a server function. See {@link http://docs.marklogic.com/vec.cosineSimilarity|vec.cosineSimilarity}
    * @method planBuilder.vec#cosineSimilarity
    * @since 3.5.0
    * @param { VecVector } [vector1] - The vector from which to calculate the cosine similarity with vector2.
    * @param { VecVector } [vector2] - The vector from which to calculate the cosine similarity with vector1.
    * @returns { XsDouble }
    */
cosineSimilarity(...args) {
    ...
    const paramdefs = [['vector1', [types.VecVector, PlanColumn, PlanParam], true, false], ['vector2', [types.VecVector, PlanColumn, PlanParam], true, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'vec.cosineSimilarity', 2, new Set(['vector1', 'vector2']), paramdefs, args) :
        bldrbase.makePositionalArgs('vec.cosineSimilarity', 2, false, paramdefs, args);
    return new types.XsDouble('vec', 'cosine-similarity', checkedArgs);
...
get(...args) {
    const namer = bldrbase.getNamer(args, 'vector1');
    const paramdefs = [['vector1', [types.VecVector, PlanColumn, PlanParam], true, false], ['k', [types.XsUnsignedInt, PlanColumn, PlanParam], true, false]];
    const checkedArgs = (namer !== null) ?
...
vectorScore(...args) {
    ...
    const paramdefs = [['score', [types.XsUnsignedInt, PlanColumn, PlanParam], true, false], ['similarity', [types.XsDouble, PlanColumn, PlanParam], true, false], ['similarityWeight', [types.XsDouble, PlanColumn, PlanParam], false, false]];
    const checkedArgs = (namer !== null) ?
        bldrbase.makeNamedArgs(namer, 'vec.vectorScore', 2, new Set(['score', 'similarity', 'similarityWeight']), paramdefs, args) :
...
class PlanPatchBuilder extends types.ServerType {
  ...
  constructor(ns, fn, args) {
    ...
    /** Changing all the methods in PlanPatchBuilder to return PlanPatchBuilderBase in order to create sub-operators.
     * @since 3.4.0
     */
...
insertAfter(...args) {
    ...
    return new PlanPatchBuilderBase([new PlanPatchBuilder('op', 'insert-after', checkedArgs)]);
    }
/**
...
insertBefore(...args) {
    ...
    return new PlanPatchBuilderBase([new PlanPatchBuilder('op', 'insert-before', checkedArgs)]);
...
insertChild(...args) {
    ...
    return new PlanPatchBuilderBase(new PlanPatchBuilder('op', 'insert-child', checkedArgs));
    }
/**
...
insertNamedChild(...args) {
    ...
    return new PlanPatchBuilderBase(new PlanPatchBuilder('op', 'insert-named-child', checkedArgs));
...
remove(...args) {
    ...
    return new PlanPatchBuilderBase(new PlanPatchBuilder('op', 'remove', checkedArgs));
    }
/**
...
replace(...args) {
    ...
    return new PlanPatchBuilderBase(new PlanPatchBuilder('op', 'replace', checkedArgs));
...
replaceInsertChild(...args) {
    ...
    return new PlanPatchBuilderBase(new PlanPatchBuilder('op', 'replace-insert-child', checkedArgs));
    }
/**
...
replaceValue(...args) {
    ...
    return new PlanPatchBuilderBase(new PlanPatchBuilder('op', 'replace-value', checkedArgs));
    }
}

/**
 * Adding this class manually to help create sub-operators for patchBuilder.
 * @namespace planBuilder.PlanPatchBuilderBase
 * @since 3.4.0
 */
class PlanPatchBuilderBase{
    constructor(args){
        patchArgs.push(args);
        return new PlanPatchBuilder('op', 'suboperators', patchArgs);
    }
}
/**
...
/**
    ...
    * @param { PlanColumn } [sourceCol] - The document uri or fragment id value. This is either an op:fragment-id-col specifying a fragment id column or a document uri column as xs:string or as a column using op:col, op:view-col or op:schema-col. Joining on a fragment id is more efficient than joining on a uri column.
    * @returns { planBuilder.ModifyPlan }
    */
joinDocAndUri(...args) {
    const namer = bldrbase.getNamer(args, 'docCol');
...
patchBuilder(...args) {
    ...
    patchArgs = [];
        return new PlanPatchBuilderBase(new PlanPatchBuilder('op', 'patch-builder', checkedArgs));
    }
/**
