...
function castArg(arg, funcName, paramName, argPos, paramTypes) {
  if (arg === void 0 || arg === null || !Array.isArray(paramTypes) || paramTypes.length === 0) {
    ...
    if (paramTypes.some(paramType => (arg instanceof paramType))) {
      ...
      // add another switch for docColeIdentifier, doc-cols (these are the types in types package in java)
      console.log('line 79 in plan-builder-base: '+paramClass)
      switch(paramClass) {
      ...
      case 'PlanXsValueMap':
        ...
        case 'PlanRowColTypes':  return Object.keys(arg).every(key => {
