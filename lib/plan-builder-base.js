  ...
  if (arg === void 0 || arg === null || !Array.isArray(paramTypes) || paramTypes.length === 0) {
    ...
    if (paramTypes.some(paramType => (arg instanceof paramType))) {
      ...
      case 'PlanSearchOption':  return Object.keys(arg).every(key => {
        ...
        switch(key) {
          case 'scoreMethod':
            if (['logtfidf', 'logtf', 'simple', 'score-logtfidf', 'score-logtf', 'score-simple', 'score-bm25'].includes(value)) {
              return true;
            }
            throw new Error(
                `${argLabel(funcName, paramName, argPos)} can only be logtfidf, logtf, simple, score-logtfidf, score-logtf,
                score-simple or score-bm25`
            );
          case 'qualityWeight':
