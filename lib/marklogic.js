...
const mllog                = require('./mllog.js');

const documents            = require('./documents.js');
const Graphs               = require('./graphs.js');
const Rows                 = require('./rows.js');
const Values               = require('./values.js');
const ExtLibs              = require('./extlibs.js');
const RESTServerProperties = require('./rest-server-properties.js');
const Transactions         = require('./transactions.js');
const Transforms           = require('./transforms.js');
const ResourcesConfig      = require('./resources-config.js');
const ResourcesExec        = require('./resources-exec.js');
const Databases            = require('./databases.js');
const serverExec           = require('./server-exec.js');

const queryBuilder         = require('./query-builder.js');
const patchBuilder         = require('./patch-builder.js');
const valuesBuilder        = require('./values-builder.js');
const planBuilder          = require('./plan-builder.js');
const ctsQueryBuilder      = require('./ctsquery-builder');
...
ExtlibsWrapper.prototype.write = function writeExtlibsWrapper() {
  ...
  const module = args[0];
  const ext = mlutil.extension(module);
  if (ext === null) {
...
function expandExtlibsWrapper(action, args) {
  ...
  const module = (args.length > 0) ? args[0] : null;
...
function initClient(client, inputParams) {
  ...
  let isSSL = (inputParams.ssl == null) ? false : inputParams.ssl;
  ...
  const noAgent = (inputParams.agent == null);
  const agentOptions = noAgent ? {
    keepAlive: true
  } : null;
  if (isSSL) {
...
function releaseClient(client) {
  const agent = client.connectionParams.agent;
  if (agent == null){
    return;
  }
  agent.destroy();
